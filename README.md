...
There are infinite ways to build Tic Tac Toe. For this lesson, we want you to build it in a pretty specific way. Your code must deconstruct Tic Tac Toe into predefined methods that represent basic requirements and logic of the game. Instead of programming everything procedurally and literally, we're going to simplify our code by building very specific and simple methods that come together to create a game.

For example, imagine the following procedure for printing a board three times:
...
vs a procedure encapsulated by a method to print a board used three times:
...
When our logic is well encapsulated by individual methods everything about our code is easier.

You'll approach building Tic Tac Toe in this manner, first defining helper methods to spec and then slowly putting them together to define the complete play logic. You'll then use this play method within a CLI and build a fully functioning 2 player Tic Tac Toe CLI in Ruby.
...
#### Hint: Reading the `#play` test specs and output
...
At your prompt, you can execute ruby just like you could in an IRB session. The only difference is that your console has already read all the code in your application.
...
Included with this lesson is a plugin to our tests that will generate a Coverage Report that shows how much of our code is tested when we run our tests. After a test run, you can open the file generated in `coverage/index.html` from terminal with `open coverage/index.html`. It should pop up in your browser and look like:
